...
请仔细分析图片描述，根据图片中的具体内容（物体、动物、植物、颜色、形状等）来创建宠物角色。不要使用通用的模板，每个宠物都应该是独特的。

请创建一个完整的宠物角色设定，包括：
1. 世界设定（可以是科幻、奇幻、现实、神话等任何风格）
2. 背景故事（宠物的来历、特殊能力等，要与图片内容相关）
3. 性格特征（3-5个关键词，体现图片中物体的特点）
4. 个性描述（一段话描述，要基于图片内容）
...
重要：请确保宠物设定与图片内容高度相关，不要使用通用的描述。每个宠物都应该是独特的，基于图片中的具体内容。`;

    const messages = [
      { role: 'system', content: '你是一个创意作家，专门创作有趣的电子宠物角色设定。请确保每个角色都是独特的，基于用户提供的具体图片内容。' },
      { role: 'user', content: prompt }
    ];
